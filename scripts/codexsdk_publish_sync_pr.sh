...
Options:
  ...
  --drift-analysis <path>   Drift issue analysis markdown to include in the PR body.
...
It does not merge itself, tag, close the drift issue, or bypass branch protection. The finalize workflow owns landed verification, sync tag handling, drift verification, and issue closure after merge.
...
This PR was generated by the upstream protocol sync workflow. It stops at the protected PR boundary and does not tag, close issues, or bypass branch protection. Merge should happen only through branch protection, repository auto-merge rules, and the required \`Go\` check on this head commit.
