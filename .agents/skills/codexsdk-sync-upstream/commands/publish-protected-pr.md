...
- May request protected PR auto-merge only when caller owns that step.
- Must not push directly to protected `main`, introduce bypass tokens/statuses, tag, claim full drift resolution, delete branches, or merge around failed checks.
...
Output:
- Sync branch, PR URL/number, pushed commit when known, `action_required` status when relevant, and completion layer.
